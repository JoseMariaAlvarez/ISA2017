...
www.effectivedjango.com/tutorial/index.html,
...
```
En este caso estamos creando un modelo Contacto, que tendrá un nombre, apellidos e email. La utilidad del método `def __str___(self)` la veremos más adelante.
...
Desde el panel de administración crearemos un nuevo contacto rellenando los tres campos. Lo usaremos como ejemplo en el siguiente punto.
...
```
Como vemos, `lista_contactos` nos devuelve todos los contactos escribiendo el nombre y los apellidos. Si a la hora de crear el modelo Contacto no hubiésemos creado el método `def __str__(self)`, veríamos:
```python
...
- `{% <texto> %}` añade funciones propias del lenguaje de programación,
...
Los archivos estáticos serán todas los ficheros que no se van a modificar, como archivos *.css*, códigos *.js* o imágenes. En nuestro ejemplo anterior añadimos en el `<head></head>`:
```HTML
<link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">
```
...
Modificaremos la plantilla para hacer referencia a estos ficheros estáticos:
```HTML
<head>
  {% load staticfiles %}
  <!-- <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css"> -->
